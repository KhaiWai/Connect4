import sys
import argparse
from agents import RandomAgent, HumanAgent, MinimaxAgent, HeuristicAgent, PruneAgent, HeuristicAgent2
import test_boards
...
def play_game(player1, player2, state, depth=None):
    ...
    while not state.is_full():
        ...
        """print("Rows:", state.get_all_rows())
        print("Diags:", state.get_all_diags())

        print("\n")
        print("Col:", state.get_col(2))
        print("Row:", state.get_row(2))
        print("Diag:", state.get_diags(2,1)) """
        score = state.score()
